    ...
    <div className="bg-stone-50 p-4 sm:p-6 rounded-xl shadow-xl">
      ...
      <div className="overflow-x-auto max-h-[600px]">
        <table className="w-full text-xs sm:text-sm min-w-[700px]">
          ...
          <tbody>
            <tr>
              <td colSpan="6" className="pt-4 pb-2 text-xl sm:text-xl font-bold text-black tracking-wide">REVENUE</td>
            </tr>
            {revenueRows.map((row) => (
              <tr key={`rev-${row.category}`} className="border-t">
                ...
                <td className="text-right pr-2 sm:pr-4">
                  <span className={row.varToBudget < 0 ? 'text-red-600' : 'text-green-600'}>
                    {formatCurrency(row.varToBudget)} {getVarianceIcon(row.varToBudget)}
                  </span>
                </td>
                <td className="text-right pr-2 sm:pr-4">{formatCurrency(row.prior)}</td>
                <td className="text-right pr-2 sm:pr-4">
                  <span className={row.varToPrior < 0 ? 'text-red-600' : 'text-green-600'}>
                    {formatCurrency(row.varToPrior)} {getVarianceIcon(row.varToPrior)}
                  </span>
                </td>
              </tr>
            ))}
            ...
            <tr>
              <td colSpan="6" className="pt-4 pb-2 text-xl sm:text-xl font-bold text-black tracking-wide">EXPENSE</td>
            </tr>
            {expenseRows.map((row) => (
